...
export const confirmationEmail = async (lang: SubscriptionCollectionLanguageType, data: { link: string; }, siteConfig: SiteConfigurationType) => {
  try {
    return sprightly('dist/templates/' + siteConfig.mail.templatePath + '/confirmation_' + lang + '.html', {
      lang: lang,
      link: data.link,
    });
  } catch (error) {
...
export const expiryEmail = async (lang: SubscriptionCollectionLanguageType, data: { 
  ...
  removal_date: string,
  remove_link: string }, siteConfig: SiteConfigurationType) => {
  try {
    return sprightly('dist/templates/' + siteConfig.mail.templatePath + '/expiry_notification_' + lang + '.html', {
      lang: lang,
      link: data.link,
...
export const newHitsEmail = async (lang: SubscriptionCollectionLanguageType, data: {
  ...
  search_link: string,
  remove_link: string,
  created_date: string }, siteConfig: SiteConfigurationType) => {
  try {
