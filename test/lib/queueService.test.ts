...
describe('QueueService', () => {
  ...
  test('Leaves broadcast status records alone', { concurrency: false }, async () => {
    const db = mongo.db();
    const statusRecord = await db.collection('queue').insertOne({
      type: 'broadcast',
      site_id: 'rekry',
      status: 'completed',
      test: false,
    ...
    // The email item was drained, but the status record survives.
    const emailItem = await db.collection('queue').findOne({ _id: item.insertedId });
    assert.ok(emailItem === null, 'Queue item was deleted');
    const status = await db.collection('queue').findOne({ _id: statusRecord.insertedId });
    assert.ok(status !== null, 'Broadcast status record was not deleted');
  });
});
